...
export class NgDatepickerComponent implements ControlValueAccessor, OnInit, OnChanges {
  ...
  @Input() redEmotionDates: Date[];
  @Input() yellowEmotionDates: Date[];
  @Input() greenEmotionDates: Date[];
  ...
  days: {
    ...
    isToday: boolean;
    isSelected: boolean;
    isRedEmotion: boolean;
    isYellowEmotion: boolean;
    isGreenEmotion: boolean;
  }[];
  ...
  compareRedDate( date1: any) {
    let normalDate = new Date(date1);
    let retValue = false;
    this.redEmotionDates.forEach ( (x) => {
      let emotionDate = new Date(x);
      if (normalDate.getTime() === emotionDate.getTime()) {
        retValue = true;
      }
    });
    return retValue;
  ...
  compareYellowDate( date1: any) {
    let normalDate = new Date(date1);
    let retValue = false;
    this.yellowEmotionDates.forEach ( (x) => {
      let emotionDate = new Date(x);
      if (normalDate.getTime() === emotionDate.getTime()) {
        retValue = true;
      }
  ...
  compareGreenDate( date1: any) {
    let normalDate = new Date(date1);
    let retValue = false;
    this.greenEmotionDates.forEach ( (x) => {
      let emotionDate = new Date(x);
      if (normalDate.getTime() === emotionDate.getTime()) {
        retValue = true;
      }
  ...
  init(): void {
    ...
    this.days = eachDay(start, end).map(date => {
      return {
        ...
        isRedEmotion: this.compareRedDate(date),
        isYellowEmotion: this.compareYellowDate(date),
        isGreenEmotion: this.compareGreenDate(date)
      };
    });
